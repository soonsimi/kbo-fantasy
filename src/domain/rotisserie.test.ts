...
import type { StandingsInput } from './rotisserie';
import { EMPTY_HITTING, EMPTY_PITCHING } from './types';
import type { HittingLine, Manager, PitchingLine, PlayerId } from './types';
...
describe('scoreCategory', () => {
  ...
  it('9인 리그에서 1위는 9점, 최하위는 1점이다', () => {
    const entries = Array.from({ length: 9 }, (_, i) => ({
      managerId: `m${i}`,
      value: 100 - i * 10,
    }));
    const result = scoreCategory(entries, hr);
    expect(result.m0).toMatchObject({ points: 9, rank: 1 });
    expect(result.m8).toMatchObject({ points: 1, rank: 9 });
  });
  ...
  it('타자 삼진은 적은 쪽이 상위다', () => {
    ...
    expect(result.few).toMatchObject({ rank: 1, points: 3 });
    expect(result.mid.rank).toBe(2);
    expect(result.many).toMatchObject({ rank: 3, points: 1 });
  });
...
function buildInput(
  ...
  const rosters: Array<{ managerId: string; playerIds: string[] }> = [];
  ...
  for (const [managerId, lines] of Object.entries(teams)) {
    ...
    managers.push({ id: managerId, name: managerId.toUpperCase() });
    rosters.push({ managerId, playerIds: [playerId] });
    if (lines.hitting) hitting[playerId] = { ...EMPTY_HITTING, ...lines.hitting };
    if (lines.pitching) pitching[playerId] = { ...EMPTY_PITCHING, ...lines.pitching };
  }

  return { managers, rosters, hitting, pitching };
}

describe('aggregateRoster', () => {
  it('보유 선수의 모든 항목을 합산한다', () => {
    const hitting: Record<PlayerId, HittingLine> = {
      a: { ...EMPTY_HITTING, r: 90, h: 150, hr: 30, rbi: 100, sb: 20, bb: 60, so: 110, gdp: 12 },
      b: { ...EMPTY_HITTING, r: 50, h: 100, hr: 10, rbi: 55, sb: 5, bb: 40, so: 70, gdp: 8 },
    };

    const totals = aggregateRoster(hitting, {}, ['a', 'b']);
  ...
  it('투수 이닝은 소수 합으로 누적된다', () => {
    const pitching: Record<PlayerId, PitchingLine> = {
      a: { ...EMPTY_PITCHING, ip: 100 + 1 / 3, w: 8, so: 90 },
      b: { ...EMPTY_PITCHING, ip: 50 + 2 / 3, w: 4, so: 45 },
    };

    const totals = aggregateRoster({}, pitching, ['a', 'b']);
  ...
  it('명단에 없는 선수 id는 조용히 건너뛴다', () => {
    const hitting: Record<PlayerId, HittingLine> = {
      real: { ...EMPTY_HITTING, hr: 5, h: 30 },
    };
    const totals = aggregateRoster(hitting, {}, ['real', 'ghost']);
...
describe('computeStandings', () => {
  ...
  it('참가자가 없어도 예외 없이 빈 순위표를 만든다', () => {
    const standings = computeStandings({ managers: [], rosters: [], hitting: {}, pitching: {} });
